...
operator = {'*': 1, '+': 2, '(': 3, ')': 3} # 연산자 생성 및 우선순위 넣어줌
...
for tc in range(1, T + 1):
    ...
    for i in arr:
        ...
        elif not len(stack_operater) or i == '(' or stack_operater[-1] == '(':
            stack_operater.append(i)
# 들어온 연산자가 연산자 스택의 안의 가장 위 연산자보다 우선순위가 높다면 추가하기(낮은 수가 높은 우선순위이므로 <)
        elif operator[i] < operator[stack_operater[-1]] and i != ')':
            stack_operater.append(i)
        else: #이부분이 어려웠음!!!
